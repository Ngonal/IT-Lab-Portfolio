...
## Common Commands
### Cisco IOS / IOS XE
- **`shutdown`** — Changes the administrative state of an interface. `shutdown` disables the interface, placing it into an `administratively down` link status and preventing any Layer 1 or Layer 2 activity. `no shutdown` enables the interface, allowing it to attempt link establishment.
