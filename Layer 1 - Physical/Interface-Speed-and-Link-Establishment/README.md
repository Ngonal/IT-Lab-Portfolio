...
| 2 | `show interfaces status` on `SW4` reveals `Gi1/0/1` is hard-coded to 100 Mbps — confirms the speed mismatch that caused the initial link failure | Issued `speed auto` on `SW4`'s `Gi1/0/1`; confirmed with `show interfaces Gi1/0/1 status` | Both interfaces now set to autonegotiate; configuration normalized on both switches | <img src="Elements/step2.png"> |
