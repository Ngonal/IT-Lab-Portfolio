...
Content is structured using the **`TCP/IP 5-layer Internet model`** (Internet Protocol Suite), with each example categorized by the highest layer it meaningfully interacts with. Labs/Projects are further aligned to common IT lifecycle scenarios, including **Provisioning**, **Maintenance**, and **Troubleshooting**.
...
| 3 - Network (Internet) | Routers, Layer 3 (Multilayer) Switches, Firewalls (Stateless) | IPv4, IPv6, IPSec, ICMP, IGMP, APIPA, SLAAC, BGP, OSPF, EIGRP, IS-IS, RIP, VRF, NAT, MLPS, HSRP, VRRP, GLBP, ACLs, Subnetting, CIDR, FLSM, VLSM | 2 | [View](Layer%203%20-%20Network) |
